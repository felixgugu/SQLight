...
### 4. 多結果歷史分頁與 AG Grid
- **自動智慧分頁命名**：
  - 每次執行查詢自動產生獨立的 Result 分頁，分頁名稱自動識別並擷取 SQL 中第一個涉及的資料表名稱（如 `Users`、`Orders`），簡潔明瞭。
- **釘選保護機制 (Pin / Unpin)**：
  - 點擊分頁左側圖釘即可釘選；**被釘選的分頁會自動移動至最左側**，受特殊保護，即使超過歷史保留上限也不會被自動清理。
- **嚴謹的分頁排列順序**：
...
| 滑鼠拖曳 (<kbd>Pointer Drag</kbd>) | Results 分頁 | **拖曳排序**：按住結果分頁左右拖動可自由調整排列順序（無禁止符號） |
